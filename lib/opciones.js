...
export const URGENCIAS = [
  { value: 'inmediata', label: 'Para empezar hoy o mañana' },
  { value: 'esta_semana', label: 'Esta semana' },
  { value: 'este_mes', label: 'Este mes' },
  { value: 'sin_apuro', label: 'Sin apuro' },
];

// "hoy" era la opción anterior ("Urgente, para esta semana"). Las vacantes que la
// tienen guardada se tratan como "esta semana".
export function urgenciaActual(valor) {
  return valor === 'hoy' ? 'esta_semana' : valor;
}

// "Para empezar hoy o mañana" se muestra como "Arrancá hoy" solo durante las
// primeras 48 horas desde la publicación; después pasa a "Busca con urgencia",
// para que un aviso viejo no siga diciendo que arranca hoy.
export const HORAS_URGENCIA_INMEDIATA = 48;

export function esInmediata(vacante) {
  if (vacante?.urgencia !== 'inmediata' || !vacante.created_at) return false;
  const horas = (Date.now() - new Date(vacante.created_at).getTime()) / (1000 * 60 * 60);
  return horas < HORAS_URGENCIA_INMEDIATA;
}
